...
function name(path) {
  ...
  const filePathArr = parts.slice(1, 3) // 可以根据接口地址，具体配置
  // 判断是否是小程序和app接口
  if (filePathArr.length > 1 && (filePathArr[1].startsWith('mini') || filePathArr[1].startsWith('app')))
    return null
...
function config(serviceName, serverUrl) {
  return {
    ...
    outputFilePath: (interfaceInfo, changeCase) => {
      ...
      const filePath = filePathArr?.map(item => changeCase.paramCase(item)).join('/')
...
export default defineConfig([
  // config('law-firm', 'http://192.168.196.232:9101/law-firm/v3/api-docs'),
  // config('member-select', 'http://192.168.196.121:8086/member-select/v3/api-docs'),
  config('document', 'http://192.168.196.121:8086/member-select/v3/api-docs'),

  // config('test', 'http://192.168.196.185:8081/v3/api-docs'),
